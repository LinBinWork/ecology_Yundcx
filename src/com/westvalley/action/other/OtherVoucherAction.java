 ...
 * 主要功能：1.生成会计凭证
 */
public class OtherVoucherAction implements Action {
    ...
    private String doAct(RequestInfo request) {
        ...
        String bxsy = wfMainMap.get("bxsy").length() > 5 ? wfMainMap.get("bxsy").substring(0, 3) : wfMainMap.get("bxsy");//报销事由
        parameter.put("BKTXT", "费用报销：_" + userName + "_" + bxsy);//抬头文本
        ...
        for (Map<String, String> map : wfMainMapList) {
            ...
            map2.put("KOSTL", map.get("fycdbm"));//成本中心
            map2.put("RSTGR", "");//现金流
