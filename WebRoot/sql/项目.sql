...
create view wv_v_proj1moreTemp as (
    ...
    from formtable_main_18 a
    join formtable_main_18_dt2 b on a.id = b.mainid
...
create view wv_v_proj1thing as (
    ...
    c.id as projid
    from formtable_main_18 a
    join formtable_main_18_dt1 b on a.id = b.mainid
...
create view wv_v_contractFlow as (
    ...
    from (
      select a.requestid,c.NAME1 as vendorName,a.gysmc as vendorNo, 'Y' as type  from formtable_main_26 a--营销合同
      join OA_SAP_GYS c on a.gysmc = c.PARTNER
      UNION ALL
      select a.requestid,a.dfgsmc,'' as vendorNo, 'N' as type  from formtable_main_20 a--收款合同
      UNION ALL
...
create view wv_v_contractinfo as (
    ...
    join formtable_main_26_dt2 b on a.id = b.mainid
